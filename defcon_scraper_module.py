...
# def get_defcon_current_level_news():
#
#     out_file = './defcon_news_current_news.txt'
#     _text = []
#
...
#                 debug_output.append('[' + str(datetime.datetime.now()) + '] [DEFCON] [CURRENT LEVEL NEWS] ' + str(e))
#     else:
...
def write_defcon_levels():
    ...
    if checks_passed is True:
        ...
        if bool_write is True:
            ...
            open('./defcon_level.txt', 'w').close()
            with open('./defcon_level.txt', 'a') as fo:
...
def defcon_news(save_news=False):
    ...
    try:
        ...
        for _ in url_item:
            ...
            out_file = './defcon_news_' + article_title.replace('/', '') + '.txt'
